...
from PA2.ttypes import NodeInfo
...
class NodeHandler:
  ...
  def closest_preceding_finger(self, id):
    for i in range(chordLen)[::-1]: # i is [m-1,0], i+1 -> [m,1]
      if between(self.finger[i+1].id, self.id, id):
        return NodeInfo(self.finger[i+1].id, self.finger[i+1].ip, self.finger[i+1].port)
    
    result = NodeInfo(self.id, self.ip, self.port)
    return result
     
  # join from another arbitrary node in the network
  def join(self, givenNode):
    if self.id == givenNode.id: # self is the only node in the network
      print("There is no other node in the DHT.")
      print(chordLen)
      for i in range(chordLen):
        self.finger[i+1] = NodeInfo(self.id, self.ip, self.port)
      
      self.predecessor = NodeInfo(self.id, self.ip, self.port)
      print(self.finger)
    else:
      self.init_finger_table(givenNode)
      self.update_others()  
  ...
  def init_finger_table(self, newNodeInfo):
    ...
    suctransport.open()
    print(self.start[0]) #99
    suc = succlient.findSuccessor(self.start[0])
    print("finish finding the successor." + str(suc.id) + suc.ip + str(suc.port))
    # update the finger table [0] {id: NodeInfo}
    self.finger[0] = suc
    # self.successor = finger[0] - NodeInfo = suc

    print("start connecting to self.successor")
  ...
  ## after joining the DHT, the new node will contact nodes in the finger table to let them update DHT
  def UpdateDHT(self):
    self.update_others()
...
def ConnectSuperNode(IP, port):
  ...
  print(superNodeIP + str(superNodePort))
...
def startNodeServer(id, ip, port):
  handler = NodeHandler(id,ip,port)
...
if __name__ == '__main__':
  ...
  currNode = NodeHandler(id, nodeIP, nodePort)
  ...
  if (info == "NACK"):
    ...
    #startNodeServer(currNode.id, currNode.ip, currNode.port)
  ...
  info = superNodeclient.PostJoin(nodeIP, str(nodePort), id) 
  # Close!
  transport.close()

  print("Node " + str(id) + " is serving now")
  startNodeServer(id, nodeIP, nodePort)
